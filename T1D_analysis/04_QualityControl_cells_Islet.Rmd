---
title: "04_QualityControl_cells_Islet"
...
script_name <- "04_QualityControl_cells_Islet.Rmd"
...
- Batch correction was not run on the current SPE object, when it is done add
`fastMNN` to the `assay_sel <- ` assignments.  

- Due to the high number of cells, parallelization can cause memory issues. In
particular, plotting on reduced dimensions may have to be done sequentially.

- Other quality checks diagnostics could be added, see https://bodenmillergroup.github.io/IMCDataAnalysis/image-and-cell-level-quality-control.html#seg-quality  
...
corr_pca$counts
print(corr_pca$count, n = "all")
```

Results: Significant contributions are observed for batch & stages only (to PC1/PC2).
For age: association to PC2.
...
plot_list <- purrr::map2(pcas, names(pcas), \(cur_pc, cur_assay) {
  purrr::walk(var_to_plot, \(cur_var) {
    ...
    p <- as_tibble(cur_pc, rownames = "case_id") |> 
      ...
      geom_point(aes(color = get(cur_var))) +
...
Cell-based signal-to-noise ratio (SNR) is calculated for each marker.
```{r snr, warning=FALSE, message=FALSE}
# Calculate signal-to-noise ratio (SNR) for each marker.
library(mclust)
...
fn <- paste0(paste(today, "SignalToNoiseRatio", sep = "_"), ".png")
do.call(ggsave, c(list(fn, p), plotsave_param_large))
```

Check Cell area.
```{r cell-area-plot}
p <- dittoPlot(spe, var = "cell_area", 
          group.by = "case_id", 
          plots = "boxplot") +
        ylab("Cell area") + 
        xlab("")

fn <- paste0(paste(today, "CellArea", sep = "_"), ".png")
do.call(ggsave, c(list(fn, p), plotsave_param_large))
```

Check number of very small and very large cells. 
These are few (~ 500). 
Could remove, but not necessary in this case, given that they are 
< 0.1 % of the total number of cells.
```{r cell-area-filter}
# Remove small cells.
# sum(spe$cell_area < 5)
# sum(spe$cell_area >= 300)
# spe <- spe[,spe$area >= 5 & spe$area < 300]
```
...
set.seed(seed)
image_list <- mask_list <- list.files(folder_images)
image_list2 <- mask_list2 <- sort(sample(unique(image_list), nb_images))
writeLines(c("Selected images:", image_list2))
```
...
# Subset the SPE object
spe_sub <- spe[, spe$image_fullname %in% image_list2]
...
cytomapper::plotPixels(images, object = spe_sub,
                       masks, cell_id = "cell_number", img_id = "ImageName",
                       colour_by = c("CD3", "CD163"),
                       outline_by = "cell_area",
                       bcg = list(CD3 = c(0, 5, 1), CD163 = c(0, 5, 1)),
                       image_title = NULL, scale = TRUE, save_plot = fn)
```
...
```{r spillover-plot}
# Subset images to 14 random images.
set.seed(seed)
plot_img_compensation <- TRUE
image_ids <- sample(seq_len(length(images)), nb_images)

images2 <- cytomapper::getImages(images, image_ids) 
images2_comp <- cytomapper::getImages(images_comp, image_ids)

if (isTRUE(plot_img_compensation)) {
  # In115
  fn <- list(filename = file.path(plotsave_param$path, paste0(
    paste(today, "Cytomapper", "SpillComp", "In115-before", sep = "_"), ".png")))
  
  cytomapper::plotPixels(images2, colour_by = "In115Di", 
                         bcg = list(In115Di = c(0, 4, 1)), legend = NULL, save_plot = fn)
  cytomapper::plotPixels(images2[10], colour_by = "In115Di",
  ...
  cytomapper::plotPixels(images2_comp, colour_by = "In115Di",
                         bcg = list(In115Di = c(0, 4, 1)), legend = NULL, save_plot = fn)
  cytomapper::plotPixels(images2_comp[10], colour_by = "In115Di",
  ...
  cytomapper::plotPixels(images2_comp, colour_by = "Nd142Di",
                         bcg = list(Nd142Di = c(0, 4, 1)), legend = NULL, save_plot = fn)
  cytomapper::plotPixels(images2_comp[10], colour_by = "Nd142Di",
  ...
  cytomapper::plotPixels(images2, colour_by = "Nd146Di",
                         bcg = list(Nd146Di = c(0, 4, 1)), legend = NULL, save_plot = fn)
  cytomapper::plotPixels(images2[10], colour_by = "Nd146Di",
  ...
  cytomapper::plotPixels(images2_comp, colour_by = "Nd146Di",
                         bcg = list(Nd146Di = c(0, 4, 1)), legend = NULL, save_plot = fn)
  cytomapper::plotPixels(images2_comp[10], colour_by = "Nd146Di",
...
if (isTRUE(do_img_compensation)) {
  ...
  channelNames(images_comp) <- original_channel_names[!original_channel_names %in% c("DNA1", "DNA3")]

  # Save the compensated images
  fn <- file.path(paths$folder_in, "spillcomp_adapted", paths$panel_type)
