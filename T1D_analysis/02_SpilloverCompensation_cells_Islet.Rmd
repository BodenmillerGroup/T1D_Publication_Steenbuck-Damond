---
title: "02_SpilloverCompensation_cells_Islet"
...
script_name <- "02_SpilloverCompensation_cells_Islet.Rmd"
...
compensation slide where each spot contains a single metal isotope. Here, this 
spillover measured and correction is applied to the measured cell counts, as 
...
bin_size <- list("Batch1" = 3, "Batch2" = 4,
                 "Batch3" = 5, "Batch4" = 4)
...
if (!("raw" %in% assayNames(spe))) {
  ...
  for (i in seq_along(batches)) {
    ...
    metadata(spe_comp[[i]]) <- list(spillover_matrix = metadata(spillcomp_bin[[i]])$spillover_matrix)
  }
...
```{r select-channels}
channel_sel <- rownames(rowData(spe)[rowData(spe)$metal %in% c("La139", "Gd155", "Ce140", "Nd142", "In113", "In115"), ])
```
...
# Note: The SPE is subsetted to batches 2 & 4 only, as there was considerate spillover between 
# La139 and Gd155 in batch 4. (Spillover = 20 %)
# Channels in Islet panel correspond to SST and Pdx1.

spe_batch <- spe_sub[, c(which(spe_sub$batch == "4"), which(spe_sub$batch == "2"))]

p1 <- dittoScatterPlot(spe_batch, x.var = "SST", y.var = "PDX1",
                       assay.x = "exprs_raw", assay.y = "exprs_raw",
                       size = 0.3) +
  ggtitle("Before compensation")

p2 <- dittoScatterPlot(spe_batch,  x.var = "SST", y.var = "PDX1",
                       assay.x = "exprs_comp", assay.y = "exprs_comp",
                       size = 0.3) +
  ggtitle("After compensation")

print(p1 + p2)
fn <- paste0(paste("CorrectedCounts", "SST", "PDX1", "batch4",
                   sep = "_"), ".png")
...
p1 <- dittoScatterPlot(spe_batch1, x.var = "SST", y.var = "PDX1",
                       assay.x = "exprs_raw", assay.y = "exprs_raw",
                       size = 0.3) +
  ggtitle("Before compensation")

p2 <- dittoScatterPlot(spe_batch1,  x.var = "SST", y.var = "PDX1",
                       assay.x = "exprs_comp", assay.y = "exprs_comp",
                       size = 0.3) +
  ggtitle("After compensation")
...
fn <- paste0(paste("CorrectedCounts", "SST", "PDX1", "batch1",
                   sep = "_"), ".png")
do.call(ggsave, c(list(fn, p1 + p2), plotsave_param))
```
...
p1 <- dittoScatterPlot(spe_sub, x.var = "H3", y.var = "HBP",
                       assay.x = "exprs_raw", assay.y = "exprs_raw",
                       size = 0.3) +
  ggtitle("Before compensation")

p2 <- dittoScatterPlot(spe_sub,  x.var = "H3", y.var = "HBP",
                       assay.x = "exprs_comp", assay.y = "exprs_comp",
                       size = 0.3) +
  ggtitle("After compensation")

print(p1 + p2)
fn <- paste0(paste("CorrectedCounts", "H3", "HBP",
                   sep = "_"), ".png")
do.call(ggsave, c(list(fn, p1 + p2), plotsave_param))

#---------------------------------------------
# Plot channels 5 and 6
p1 <- dittoScatterPlot(spe_sub, x.var = "INS", y.var = "CHGA",
                       assay.x = "exprs_raw", assay.y = "exprs_raw",
                       size = 0.3) +
  ggtitle("Before compensation")

p2 <- dittoScatterPlot(spe_sub, x.var = "INS", y.var = "CHGA",
                       assay.x = "exprs_comp", assay.y = "exprs_comp",
                       size = 0.3) +
  ggtitle("After compensation")

print(p1 + p2)
fn <- paste0(paste("CorrectedCounts", "INS", "CHGA",
                   sep = "_"), ".png")
do.call(ggsave, c(list(fn, p1 + p2), plotsave_param))
```
