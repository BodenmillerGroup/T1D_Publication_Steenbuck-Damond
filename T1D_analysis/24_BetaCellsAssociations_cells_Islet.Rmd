...
### Upregulated: HLA-ABC, B2M, ADAR (+), CHGA (+), ERN1, Ki67.
### Note: Ki67 has hard to detect expression levels in beta-cells (likely none). 
temp <- exprs_dat |> 
  filter(cell_type == "Beta") |> 
...
Note: all these are not significant.

From mAAb+ + REC all are downregulated (Ns or sig.) in comparison to ND. 
...
Do not consider disease-duration, as model fit across stages.
...
names(palettes$stages2) <- c("Control", "sAAb+", "mAAb+", "Onset", "LD", "Pancreatitis")

q <- grs2 |> 
  filter(!is.na(GRS2_score)) |> 
  filter(donor_type %in% c("NoDiabetes", "sAAb+", "mAAb+", "RecentOnset", "LongDuration")) |>
  mutate(donor_type = factor(donor_type, levels = metadata(spe)$stages)) |>
  ggplot(aes(x = donor_type, y = GRS2_score, fill = donor_type)) +
  geom_boxplot() +
  geom_point()+
  ...
  theme(legend.position = "none")  # scale colors by stage

q
saveRDS(q, file.path(paths$home_git, "figures", "rev.rds"))
...
rev_supplfig1b <- p
saveRDS(p, file.path(paths$home_git, "figures", "rev_supplfig1b.rds"))
```

-> Markers previoulsy highly significant, remain highly sig. after adjustments.
```{r}
beta_cells |> 
  tidyr::nest(data = -c(channel)) |>
...
rev_supplfig1a <- fig
saveRDS(rev_supplfig1a, file.path(paths$home_git, "figures", "rev_supplfig1a.rds"))
```
...
-> Removed, as unused. 
In part, effect due to lower BMI of children.
...
# summarize and group - as for 6496 we have two GADA titers (these are very close together).
...
saveRDS(p, file.path(paths$home_git, "figures", "rev_supplfig4.rds"))
```


## A.3.3: Spillover-free test for TXNIP.

Remove beta-cells that are next to Acinar cells, and then test again for association.

-> Analysis removed, as unused in manuscript. 

**-> Results: TXNIP corelation to GADA index even after removing many beta-cells, that are next to Acinar cells. 
This makes lateral spillover from acinar cells much less likely.**